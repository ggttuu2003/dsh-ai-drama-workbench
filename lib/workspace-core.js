...
var CHARACTER_VISUAL_SLOT_KEYS = ["turnaround", "costume", "reference"];
...
async function buildCharacterLooks(characterDirectory, index) {
  ...
  const looks = await Promise.all(lookDirectories.map(async (directory) => {
    const document = (index.filesByDirectory.get(directory.absolutePath) ?? []).find((file) => file.name === CHARACTER_LOOK_DOCUMENT);
    ...
    return {
      ...
      cover: confirmedVisuals.turnaround ?? confirmedVisuals.costume ?? confirmedVisuals.reference ?? pickCover(slots, ["turnaround", "costume", "reference"]),
...
function parseShotDirectoryName(directoryName) {
...
function parseShotCharacterOverrides(markdown) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || !Array.isArray(parsed.overrides)) return [];
    return parsed.overrides.flatMap((entry) => {
...
function parseStoredShotDesign(markdown, sceneId, directoryName) {
...
async function buildCharacterAssets(index) {
  ...
  const characters = await Promise.all(characterDirectories.map(async (directory) => {
    const profile = (index.filesByDirectory.get(directory.absolutePath) ?? []).find((file) => file.name === "\u89D2\u8272\u8BBE\u5B9A.md");
    ...
    return {
      ...
      looks,
      cover: confirmedTurnaround ?? confirmedVisuals.costume ?? confirmedVisuals.reference ?? pickCover(slots, ["turnaround", "costume", "reference"]),
...
async function buildSimpleDocumentAssets(index, parentName, documentName, slotDefinitions, type) {
  ...
  return Promise.all(directories.map(async (directory) => {
    const document = (index.filesByDirectory.get(directory.absolutePath) ?? []).find((file) => file.name === documentName);
...
async function buildSceneAssets(index, storedShots, locations, props) {
  ...
  const scenes = await Promise.all(sceneDirectories.map(async (directory) => {
    const sceneFile = (index.filesByDirectory.get(directory.absolutePath) ?? []).find((file) => file.name === "\u573A\u6B21.md");
...
async function readProjectJsonSnapshot(root) {
  ...
  if (raw.schemaVersion !== 1 || raw.rootName !== path.basename(root) || !raw.projectSettings || arrays.some((key) => !Array.isArray(raw[key]))) {
...
function validateShotDesign(design) {
  return {
...
var MODELED_SHOT_SECTIONS = /* @__PURE__ */ new Set(["\u753B\u9762\u63CF\u8FF0", "\u53F0\u8BCD", "\u63D0\u793A\u8BCD", "\u8D1F\u9762\u63D0\u793A\u8BCD", "\u4EBA\u7269\u9020\u578B\u8986\u76D6", "\u6765\u6E90\u5173\u8054"]);
...
function isUnconfiguredSceneDocument(content) {
  const remaining = content.replace(/^# .*场次资产\s*$/gmu, "").replace(/^## 场次说明\s*$/gmu, "").replace(/^- \*\*来源脚本：\*\*.*$/gmu, "").replace(/^- \*\*制作状态：\*\* 待准备\s*$/gmu, "").replace(/^- \*\*说明：\*\* 在这里补充本场的空间关系、统一视觉、连续性和交付要求。\s*$/gmu, "").trim();
  return !remaining;
}
function serializeSceneImagePrompt(sceneId, shot, sourcePath) {
  const prompt = (shot.design.prompt || shot.design.content || "").trim();
  const sourceLabel = [shot.design.shotId, shot.design.title].filter(Boolean).join(" \xB7 ");
  return [
  ...
  const target = await createAssetDirectory(
    ...
    async (directory) => {
      await fs.writeFile(
        ...
        serializeShotDesign(design, void 0, source),
        { flag: "wx" }
      );
    },
