...
const WORKFLOW_PRESETS = deepFreeze([
  ...
  },
  {
    ...
    referenceImagesEnabled: true,
    maxReferenceImages: 2,
    // `referenceImage` is the legacy/base input. `referenceImage2` is an
    // optional second image and is wired to the same graph with a fallback to
    // the base image for old one-image submissions.
    referenceImageRoles: ['referenceImage', 'referenceImage2'],
    defaults: { width: 1536, height: 864, denoise: 0.65 },
    output: { kind: 'image', targetSlots: [{ assetType: 'shot', slot: 'firstFrame' }] },
    inputs: standardImageToImageInputs(),
