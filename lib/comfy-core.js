...
// Reference selections are intentionally bounded. A workflow may expose a
// smaller limit, but never more than this many files through the workbench
// contract.
export const COMFY_MAX_REFERENCE_IMAGES = 2
...
const WORKFLOW_PRESETS = deepFreeze([
  ...
  {
    id: 'scene-image-img2img-v1',
    label: '场景图（参考生图）',
    referenceImagesEnabled: true,
    maxReferenceImages: 2,
    referenceImageRoles: ['referenceImage', 'referenceImage2'],
    defaults: { width: 1536, height: 864 },
    output: {
      kind: 'image',
      targetSlots: [
    ...
    inputs: flux2ReferenceImageInputs(),
    uploadRoles: [
      { role: 'referenceImage', required: true, mediaKind: 'image' },
      { role: 'referenceImage2', required: false, mediaKind: 'image' },
    ],
  },
  {
    id: 'prop-image-v1',
...
function flux2ReferenceImageInputs() {
  return [
    { key: 'prompt', type: 'string', required: true, maxLength: 20_000 },
    { key: 'negativePrompt', type: 'string', required: false, maxLength: 20_000 },
    { key: 'width', type: 'integer', required: false, minimum: 256, maximum: 2048 },
    { key: 'height', type: 'integer', required: false, minimum: 256, maximum: 2048 },
    { key: 'seed', type: 'integer', required: false, minimum: 0, maximum: Number.MAX_SAFE_INTEGER },
  ]
}
...
export function getComfyWorkflowPresets() {
  return WORKFLOW_PRESETS.map(preset => {
    ...
    return {
      ...
      maxReferenceImages: referenceImageLimit(preset),
      referenceImageRoles: cloneJson(referenceImageRoles(preset)),
    }
  })
}

function referenceImageLimit(preset) {
  if (!preset.referenceImagesEnabled) return 0
  if (!Number.isSafeInteger(preset.maxReferenceImages)) return 1
  return Math.max(0, Math.min(COMFY_MAX_REFERENCE_IMAGES, preset.maxReferenceImages))
}

function referenceImageRoles(preset) {
  if (!preset.referenceImagesEnabled) return []
  const roles = Array.isArray(preset.referenceImageRoles)
    ? preset.referenceImageRoles.filter(role => typeof role === 'string' && role.trim())
    : ['referenceImage']
...
function normalizeWorkflowInputs(preset, value) {
  ...
  for (const key of Object.keys(inputs)) {
    // Drop inputs removed from stable presets so queued jobs created with the
    // previous contracts remain readable and runnable after an upgrade.
    const isLegacyReferenceInput = [
      'scene-image-img2img-v1',
      'shot-first-frame-img2img-v1',
      'shot-last-frame-img2img-v1',
    ].includes(preset.id) && ['negativePrompt', 'denoise'].includes(key)
    if (isLegacyReferenceInput) continue
...
function normalizeUploads(preset, value) {
  ...
  const referenceRoles = new Set(referenceImageRoles(preset))
  const referenceLimit = referenceImageLimit(preset)
  const roles = new Set()
  const normalized = value.map(upload => {
    ...
    const allowUnselected = candidate.allowUnselected === undefined
      ? false
      : assertBoolean(candidate.allowUnselected, 'The upload allowUnselected flag is invalid.', ComfyJobError)
    if (allowUnselected && allowedRoles.get(role)?.mediaKind !== 'image') {
      throw new ComfyJobError('Only image uploads may use allowUnselected.')
    }
  ...
  const referenceCount = normalized.reduce((count, upload) => count + (referenceRoles.has(upload.role) ? 1 : 0), 0)
  if (referenceCount > referenceLimit) {
    throw new ComfyJobError(`The ${preset.id} workflow accepts at most ${referenceLimit} reference image${referenceLimit === 1 ? '' : 's'}.`)
  }
  return normalized
}
