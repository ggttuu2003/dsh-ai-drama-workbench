...
import {
  ...
  COMFY_MAX_REFERENCE_IMAGES,
  assertComfyArchiveTarget,
  ComfyArchiveError,
  ComfyConfigurationError,
  ComfyCoreError,
  ComfyJobError,
  createComfyBridgeModelsRequest,
...
const MAX_BRIDGE_MODELS = 128
const MAX_MODEL_PRESET_IDS = 32
...
const REMOTE_MODEL_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9_-]{0,119}$/u
const REMOTE_WORKFLOW_ID_PATTERN = /^[A-Za-z0-9._-]{1,256}$/u
...
function normalizeBridgeModel(value) {
  if (!isRecord(value)) throw new BridgeRequestError('Comfy Bridge 返回了无效模型目录。')
  const id = typeof value.id === 'string' ? value.id.trim() : ''
  const label = typeof value.label === 'string' ? value.label.trim() : ''
  const workflowId = typeof value.workflowId === 'string' ? value.workflowId.trim() : ''
  if (!REMOTE_MODEL_ID_PATTERN.test(id) || !label || label.length > 120 || !REMOTE_WORKFLOW_ID_PATTERN.test(workflowId)) {
    throw new BridgeRequestError('Comfy Bridge 返回了无效模型目录。')
  }
  if (!Array.isArray(value.presetIds) || !value.presetIds.length || value.presetIds.length > MAX_MODEL_PRESET_IDS) {
    throw new BridgeRequestError('Comfy Bridge 返回了无效模型目录。')
  }
  const presetIds = value.presetIds.map(item => typeof item === 'string' ? item.trim() : '')
  if (presetIds.some(item => !REMOTE_MODEL_ID_PATTERN.test(item)) || new Set(presetIds).size !== presetIds.length) {
    throw new BridgeRequestError('Comfy Bridge 返回了无效模型目录。')
  }
  if (typeof value.available !== 'boolean') throw new BridgeRequestError('Comfy Bridge 返回了无效模型目录。')
  const reason = typeof value.reason === 'string' ? value.reason.trim().slice(0, 500) : ''
  return { id, label, workflowId, presetIds, available: value.available, ...(reason ? { reason } : {}) }
}

async function getBridgeModels(profile, fetchImpl) {
  try {
    const request = createComfyBridgeModelsRequest(profile)
    const response = await bridgeFetch(fetchImpl, request, profile)
    if (!Array.isArray(response.models) || response.models.length > MAX_BRIDGE_MODELS) {
      throw new BridgeRequestError('Comfy Bridge 返回了无效模型目录。')
    }
    const models = response.models.map(normalizeBridgeModel)
    if (new Set(models.map(model => model.id)).size !== models.length) {
      throw new BridgeRequestError('Comfy Bridge 返回了重复模型目录。')
    }
    return models
  } catch (error) {
    if (error instanceof BridgeRequestError) {
      const message = error.status === 404
        ? '云端 Bridge 需要同步。'
        : '无法读取云端模型。'
      throw new ComfyApiError(502, message, 'BRIDGE_MODEL_CATALOG_UNAVAILABLE')
    }
    throw error
  }
}

async function modelsForPreset(profile, presetId, fetchImpl) {
  const models = (await getBridgeModels(profile, fetchImpl))
    .filter(model => model.presetIds.includes(presetId))
  const defaultModel = models.find(model => model.available && model.workflowId === profile.workflowMap[presetId])
    ?? models.find(model => model.available)
  return { models, defaultModelId: defaultModel?.id ?? '' }
}

async function resolveGenerationModel(profile, preset, rawModelId, fetchImpl) {
  const modelId = optionalText(rawModelId, '模型', { maximum: 120 })
  if (!modelId) return undefined
  if (!REMOTE_MODEL_ID_PATTERN.test(modelId)) throw new ComfyApiError(400, '模型无效。')
  const { models } = await modelsForPreset(profile, preset.id, fetchImpl)
  const selected = models.find(model => model.id === modelId)
  if (!selected) throw new ComfyApiError(400, '当前模型不支持此生成方式。')
  if (!selected.available) throw new ComfyApiError(400, selected.reason || '当前模型不可用。')
  return { id: selected.id, label: selected.label, remoteWorkflowId: selected.workflowId }
}
...
function selectedProjectReferenceEntries(snapshot, targetAsset, targetType) {
  ...
  const addCharacter = character => {
    add(character, ['turnaround'], 'characterReference')
    for (const look of character?.looks ?? []) {
      add(look, ['turnaround'], 'characterReference')
    }
  }
  const addLocation = location => add(location, ['setting', 'reference', 'candidate'], 'sceneReference')
  const addProp = prop => add(prop, ['reference', 'candidate'], 'propReference')
  const addScene = scene => add(scene, ['setting', 'reference', 'firstFrame', 'lastFrame', 'candidate'], 'sceneReference')
  const addShot = shot => add(shot, ['reference', 'candidate', 'firstFrame', 'lastFrame'], 'referenceImage')

  // Put the target's own selected image first when it has one, then include
  // the rest of the project so a newly prepared location can use the shot's
  // people, environment, and props in one image-to-image request.
  if (targetType === 'character') addCharacter(targetAsset)
  else if (targetType === 'location') addLocation(targetAsset)
  else if (targetType === 'prop') addProp(targetAsset)
  else if (targetType === 'scene') addScene(targetAsset)
  else if (targetType === 'shot') addShot(targetAsset)

  for (const character of snapshot?.characters ?? []) {
    if (character !== targetAsset) addCharacter(character)
  }
  for (const location of snapshot?.locations ?? []) {
    if (location !== targetAsset) addLocation(location)
  }
  for (const prop of snapshot?.props ?? []) {
    if (prop !== targetAsset) addProp(prop)
  }
  for (const scene of snapshot?.scenes ?? []) {
    if (scene !== targetAsset) addScene(scene)
  }
  for (const shot of snapshot?.shots ?? []) {
    if (shot !== targetAsset) addShot(shot)
  }
  return entries
}
...
function characterSourceForShot(snapshot, shot, scene, warnings) {
  ...
  // A look can be selected for continuity before its own visual material is
  // created. Match the workbench preview and fall back to the identity
  // reference until that look has an explicitly selected image.
  const source = look && selectedSlotImages(look, ['turnaround']).length
    ? look
    : character
  return { character, source, hasMultipleCharacters: activeCharacterPaths.size > 1 }
}
...
function effectiveCharacterSourcesForShot(snapshot, shot, scene) {
  ...
  return [...effective.values()].flatMap(({ characterPath, lookPath }) => {
    ...
    if (!character) return []
    const look = lookPath ? character.looks.find(item => item.rootPath === lookPath) : undefined
    // Keep the same fallback as automatic uploads: a selected LOOK without
    // any selected visual should use the character identity reference.
    const source = look && selectedSlotImages(look, ['turnaround']).length
      ? look
      : character
    return [source]
...
function selectedShotReferenceEntries(snapshot, shot) {
  ...
  for (const source of effectiveCharacterSourcesForShot(snapshot, shot, scene)) {
    add(imageSlotImages(source, ['turnaround']), 'characterReference')
  }
  ...
  for (const character of snapshot.characters ?? []) {
    add(imageSlotImages(character, ['turnaround']), 'characterReference')
    for (const look of character.looks ?? []) {
      add(imageSlotImages(look, ['turnaround']), 'characterReference')
    }
  }
  for (const binding of scene?.propBindings ?? []) {
    if (!binding?.propPath || !isShotInBindingRange(shot.design.shotId, binding)) continue
...
function referenceImagesOptionForPreset(preset, rawOptions) {
  ...
  if (value === true && !preset.referenceImagesEnabled) {
    throw new ComfyApiError(400, '当前工作流暂不支持参考生图，请先关闭“使用参考图”，使用基础文生图。')
  }
...
function referenceImageLimitForPreset(preset) {
  if (!preset.referenceImagesEnabled) return 0
  const rawLimit = Number.isSafeInteger(preset.maxReferenceImages)
    ? preset.maxReferenceImages
    : 1
  return Math.max(0, Math.min(COMFY_MAX_REFERENCE_IMAGES, rawLimit))
}

function referenceImageSelectionsOptionForPreset(preset, rawOptions) {
  if (rawOptions !== undefined && !isRecord(rawOptions)) {
    throw new ComfyApiError(400, '生成参数必须是对象。')
  }
  const options = rawOptions ?? {}
  const suppliedKeys = ['referenceImages', 'referenceImagePaths', 'referenceImagePath']
    .filter(key => hasOwnProperty(options, key))
  ...
  if (suppliedKeys.length > 1) {
    throw new ComfyApiError(400, '参考图参数只能使用一种格式。')
  }

  const key = suppliedKeys[0]
  let selections
  if (key === 'referenceImagePath') {
    selections = [{ path: requireText(options.referenceImagePath, '参考图路径', { maximum: 1_024 }) }]
  } else if (key === 'referenceImagePaths') {
    if (!Array.isArray(options.referenceImagePaths)) {
      throw new ComfyApiError(400, '参考图路径必须是数组。')
    }
    if (options.referenceImagePaths.length > COMFY_MAX_REFERENCE_IMAGES) {
      throw new ComfyApiError(400, `一次最多选择 ${COMFY_MAX_REFERENCE_IMAGES} 张参考图。`)
    }
    selections = options.referenceImagePaths.map((value, index) => ({
      path: requireText(value, `第 ${index + 1} 张参考图路径`, { maximum: 1_024 }),
    }))
  } else {
    if (!Array.isArray(options.referenceImages)) {
      throw new ComfyApiError(400, '参考图必须是数组。')
    }
    if (options.referenceImages.length > COMFY_MAX_REFERENCE_IMAGES) {
      throw new ComfyApiError(400, `一次最多选择 ${COMFY_MAX_REFERENCE_IMAGES} 张参考图。`)
    }
    selections = options.referenceImages.map((value, index) => {
      if (typeof value === 'string') {
        return { path: requireText(value, `第 ${index + 1} 张参考图路径`, { maximum: 1_024 }) }
      }
      if (!isRecord(value)) throw new ComfyApiError(400, `第 ${index + 1} 张参考图格式无效。`)
      const pathValue = requireText(value.path, `第 ${index + 1} 张参考图路径`, { maximum: 1_024 })
      const role = value.role === undefined || value.role === null || value.role === ''
        ? undefined
        : requireText(value.role, `第 ${index + 1} 张参考图角色`, { maximum: 128 })
      return { path: pathValue, ...(role ? { role } : {}) }
    })
  }

  if (selections.length > COMFY_MAX_REFERENCE_IMAGES) {
    throw new ComfyApiError(400, `一次最多选择 ${COMFY_MAX_REFERENCE_IMAGES} 张参考图。`)
  }
  const limit = referenceImageLimitForPreset(preset)
  if (selections.length > limit) {
    throw new ComfyApiError(400, `当前工作流最多支持 ${limit} 张参考图。`)
  }
  const paths = new Set()
  const roles = new Set()
  for (const selection of selections) {
    if (paths.has(selection.path)) throw new ComfyApiError(400, '同一张参考图不能重复选择。')
    paths.add(selection.path)
    if (selection.role) {
      if (roles.has(selection.role)) throw new ComfyApiError(400, `参考图角色“${selection.role}”不能重复。`)
...
function readPromptProperty(asset, keys) {
  if (!asset || typeof asset !== 'object') return { present: false, value: '' }
  for (const key of keys) {
    if (!Object.prototype.hasOwnProperty.call(asset, key)) continue
    const raw = asset[key]
...
function derivePrompt(asset, assetType, look, presetId) {
  if (assetType === 'character') {
    const isCostume = presetId === 'character-costume-v1' || Boolean(look)
    if (isCostume) {
      const lookPrompt = readPromptProperty(look, [
        'prompt',
        'costumePrompt',
        'costume_prompt',
        'visualPrompt',
      ...
      const lookNegative = readPromptProperty(look, [
        'negativePrompt',
        'negative_prompt',
      ...
      const lookMarkdown = typeof look?.documentContent === 'string' ? look.documentContent : ''
      const parsedLook = readCharacterLookPromptFields(lookMarkdown)
      const costumePrompt = lookPrompt.present
        ? (lookPrompt.value
          ? buildCharacterCostumePrompt(asset.profileContent || '', lookMarkdown, lookPrompt.value)
          : '')
        : buildCharacterCostumePrompt(asset.profileContent || '', lookMarkdown)
      return {
        // Keep canonical snapshot fields authoritative, including an explicit
        // empty value. Older snapshots are filled from their Markdown section.
        prompt: costumePrompt,
        negativePrompt: lookNegative.present ? lookNegative.value : parsedLook.negativePrompt,
      }
    }
    const turnaroundPrompt = readPromptProperty(asset, [
      'turnaroundPrompt',
      'turnaround_prompt',
      'threeViewPrompt',
      'three_view_prompt',
    ...
    const turnaroundNegative = readPromptProperty(asset, [
    ...
    const profile = typeof asset.profileContent === 'string' ? asset.profileContent : ''
    const parsedProfile = readCharacterTurnaroundPromptFields(profile)
    return {
      prompt: turnaroundPrompt.present
        ? turnaroundPrompt.value
        : (parsedProfile.prompt || buildCharacterTurnaroundPrompt(profile)),
      negativePrompt: turnaroundNegative.present ? turnaroundNegative.value : parsedProfile.negativePrompt,
    }
  }
  if (assetType === 'scene') {
...
export function buildShotVideoBrief(snapshot, shot) {
  ...
  // Use dedicated video prompt if present; otherwise fall back to structured brief
  const videoPrompt = design.videoPrompt?.trim()
  if (videoPrompt) {
    return compactBriefText(videoPrompt, 20_000)
  }
...
function deriveUploads(snapshot, asset, assetType, look, preset, warnings, errors, useReferenceImages = false, referenceSelections = undefined) {
  ...
  const referenceRoleOrder = (Array.isArray(preset.referenceImageRoles) && preset.referenceImageRoles.length
    ...
    : ['referenceImage'])
    .filter(role => acceptedRoles.has(role))
  const pushError = message => {
    if (!errors.includes(message)) errors.push(message)
  }

  const appendMissingRequiredUploads = () => {
    for (const definition of preset.uploadRoles) {
      ...
      if (preset.id === 'shot-first-frame-img2img-v1') {
        pushError('镜头首帧参考生图需要一张参考图。请在生成窗口选择镜头、场景、人物或道具图片。')
      } else if (preset.id === 'shot-last-frame-img2img-v1') {
        pushError('镜头尾帧参考生图需要至少一张参考图。请在生成窗口选择人物、场景、道具或镜头图片。')
      } else if (preset.id === 'scene-image-img2img-v1') {
        pushError('场景图参考生图需要至少一张参考图。请在生成窗口选择人物、场景或道具图片。')
      } else {
  ...
  // selected reference unless a dedicated image-to-image preset opts in. Keep
  // the required-upload check active, though, so a caller cannot turn off
  // references on an image-to-image preset and enqueue a job that the Bridge
  // will reject later.
  if (preset.output.kind === 'image' && !useReferenceImages) {
  ...
  if (referenceSelections !== undefined && (assetType === 'scene' || assetType === 'location')) {
    ...
    for (const selection of referenceSelections) {
      ...
      const role = [selection.role, entry.role, ...referenceRoleOrder]
        .filter(Boolean)
        .find(candidate => acceptedRoles.has(candidate) && !uploads.some(upload => upload.role === candidate))
      if (!role) {
    ...
    if (referenceSelections.length && uploads.length) warnings.push('场景图参考生图将使用指定参考图。')
    appendMissingRequiredUploads()
    return uploads
  }
  ...
  if (assetType === 'character') {
    ...
    const preferredSlots = ['turnaround']
    ...
    if (referenceSelections !== undefined) {
      ...
      for (const selection of orderedSelections) {
        ...
        if (uploads.some(upload => upload.role === role)) {
      ...
      if (referenceSelections.length && uploads.length) {
        if (preset.id === 'shot-first-frame-img2img-v1') warnings.push('首帧参考生图将使用指定参考图。')
        else if (preset.id === 'shot-last-frame-img2img-v1') warnings.push('尾帧参考生图将使用指定参考图。')
      }
      appendMissingRequiredUploads()
      return uploads
    }
    ...
    const characterImage = characterSource
      ? selectedCharacterVisual(characterSource.source, ['turnaround'], errors)
      : undefined
    ...
    if (preset.id === 'shot-first-frame-img2img-v1') {
      const primaryCandidates = [
        [shotReference, '首帧参考生图将使用当前镜头已选参考图。'],
        [sceneImage, locationImage
          ? '首帧参考生图将使用当前场次地点/环境的已选场景图。'
          : '首帧参考生图将使用当前场次已选场景图。'],
        [characterImage, '首帧参考生图将使用本镜头出场人物的已选视觉图。'],
        [propImage, '首帧参考生图将使用本镜头道具的已选参考图。'],
      ]
      const primary = primaryCandidates.find(([file]) => file)
      // Keep the automatic fallback conservative: a character is the only
      // secondary reference selected implicitly.  Props can still be picked
      // explicitly in the dialog without unexpectedly changing old jobs.
      const candidates = primary
        ? [primary, ...(characterImage && characterImage !== primary[0]
          ? [[characterImage, '首帧参考生图将使用本镜头出场人物的已选视觉图。']]
          : [])]
        : []
      ...
      for (const [file, warning] of candidates) {
        if (!file || usedPaths.has(file.path) || selectedCount >= referenceRoleOrder.length) continue
        const role = referenceRoleOrder[selectedCount]
        if (!role) break
        add(role, file)
        if (uploads.some(upload => upload.role === role)) {
      ...
      const candidates = [
        [shotReference, '尾帧参考生图将使用当前镜头已选参考图。'],
        [sceneImage, locationImage
          ? '尾帧参考生图将使用当前场次地点/环境的已选场景图。'
          : '尾帧参考生图将使用当前场次已选场景图。'],
        [characterImage, '尾帧参考生图将使用本镜头出场人物的已选视觉图。'],
        [propImage, '尾帧参考生图将使用本镜头道具的已选参考图。'],
      ]
      const usedPaths = new Set()
      let selectedCount = 0
      for (const [file, warning] of candidates) {
...
async function deriveGenerationPlan(root, body) {
  ...
  await assertComfyArchiveTarget(root, target)
  ...
  const referenceSelections = referenceImageSelectionsOptionForPreset(preset, body.options)
  const referenceImagePath = referenceSelections?.[0]?.path
  const supportsExplicitReference = (
    assetType === 'shot'
    && ['shot-first-frame-img2img-v1', 'shot-last-frame-img2img-v1'].includes(preset.id)
  ) || (
    ['scene', 'location'].includes(assetType)
    && preset.id === 'scene-image-img2img-v1'
  )
  if (referenceImagePath && !supportsExplicitReference) {
    throw new ComfyApiError(400, '指定参考图仅支持镜头或场景参考生图。')
  }
  if (referenceImagePath && !useReferenceImages) {
    throw new ComfyApiError(400, '指定参考图时不能关闭参考生图。')
  }
  const uploads = deriveUploads(snapshot, asset, assetType, look, preset, warnings, errors, useReferenceImages, referenceSelections)
...
function publicJob(job) {
  ...
  return {
    ...
    status: job.status,
    presetLabel,
    ...(job.model?.label ? { modelLabel: job.model.label } : {}),
...
export function createComfyApi(state, options = {}) {
  ...
  const handle = async (req, res, url, responseJson) => {
    ...
    try {
      ...
      if (url.pathname === `${API_PREFIX}/models` && req.method === 'GET') {
        const config = await loadConfig()
        const profileId = optionalText(url.searchParams.get('profileId'), '服务器配置', { maximum: 64 })
          ?? getPublicComfyConfig(config).activeProfile
        const presetId = requireText(url.searchParams.get('presetId'), '工作流预设', { maximum: 128 })
        const preset = getComfyWorkflowPreset(presetId)
        const profile = getRunnableComfyProfile(config, profileId)
        const catalog = await modelsForPreset(profile, preset.id, fetchImpl)
      ...
      if (url.pathname === `${API_PREFIX}/jobs/preview` && req.method === 'POST') {
        ...
        if (body.modelId !== undefined && body.modelId !== null && body.modelId !== '') {
          const config = await loadConfig()
          const profileId = optionalText(body.profileId, '服务器配置', { maximum: 64 })
            ?? getPublicComfyConfig(config).activeProfile
          model = await resolveGenerationModel(
            getRunnableComfyProfile(config, profileId),
            plan.preset,
            body.modelId,
            fetchImpl,
          )
        }
        responseJson(res, 200, { preview: { ...plan.preview, ...(model ? { modelLabel: model.label } : {}) } })
        return true
      }
      ...
      if (url.pathname === `${API_PREFIX}/jobs` && req.method === 'POST') {
        ...
        const profile = getRunnableComfyProfile(config, profileId)
        const plan = await deriveGenerationPlan(root, body)
        const model = await resolveGenerationModel(profile, plan.preset, body.modelId, fetchImpl)
