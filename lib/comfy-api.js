...
function derivePrompt(asset, assetType, look) {
  ...
  return {
    ...
    negativePrompt: String(asset.design.negativePrompt || '').trim(),
  }
}
...
function deriveUploads(snapshot, asset, assetType, look, preset, warnings, errors, useReferenceImages = false) {
...
async function deriveGenerationPlan(root, body) {
  ...
  const derivedPrompt = derivePrompt(asset, assetType, look)
  const prompt = assetType === 'shot' && preset.id === 'h3-first-last-video-v1'
    ? buildShotVideoBrief(snapshot, asset)
    : derivedPrompt.prompt
  const negativePrompt = derivedPrompt.negativePrompt
  const warnings = []
  const errors = []
  if (!prompt) errors.push('当前 Markdown 没有可用于生成的提示词或画面描述。')
  const useReferenceImages = referenceImagesOptionForPreset(preset, body.options)
  const uploads = deriveUploads(snapshot, asset, assetType, look, preset, warnings, errors, useReferenceImages)
  ...
  return {
    ...
    uploads,
    preview: {
      summary: preset.output.kind === 'image' && !useReferenceImages
...
function uploadsForRetry(job) {
  const preset = getComfyWorkflowPreset(job.workflowId)
  // Jobs created before text-to-image became the default may still contain a
  // selected reference image. Rebuild those retries without uploads so the
  // current prompt-only workflow cannot send an unsupported role to Bridge.
  if (preset.output.kind === 'image' && !preset.referenceImagesEnabled) return []
  return job.uploads.map(({ uploadId, ...upload }) => upload)
}

function uploadsForCurrentPreset(job) {
  const preset = getComfyWorkflowPreset(job.workflowId)
  // A queued record can predate the text-to-image default. Strip its stale
  // reference before recovery or execution, even if the user never clicks
  // the retry button.
  if (preset.output.kind === 'image' && !preset.referenceImagesEnabled) return []
  return job.uploads
}
...
export function createComfyApi(state, options = {}) {
        ...
        for (const job of jobs) {
          ...
          if (['uploading', 'downloading', 'archiving'].includes(job.status)
            ...
            await transitionFailed(
              ...
              job.id,
              'RECOVERY_REQUIRED',
              '工作台在本地任务未完成时重启。为避免重复提交或重复归档，请确认资料后重试。',
            )
          }
        }
  ...
  const handle = async (req, res, url, responseJson) => {
    ...
    try {
      ...
      const action = new RegExp(`^${API_PREFIX}/jobs/(job_[A-Za-z0-9-]{8,128})/(retry|cancel)$`, 'u').exec(url.pathname)
      if (action && req.method === 'POST') {
        ...
        if (!['failed', 'cancelled'].includes(job.status)) {
          throw new ComfyApiError(409, '只有失败或已取消的任务可以重新排队。', 'JOB_NOT_RETRYABLE')
        }
        if (executing.has(id)) {
          throw new ComfyApiError(409, '任务仍在本地执行，暂时不能重新排队。', 'JOB_ALREADY_RUNNING')
        }
        const retryUploads = uploadsForRetry(job)
        if (retryUploads.some(upload => !upload.sourcePath)) {
          throw new ComfyApiError(409, '该任务缺少可重新上传的本地参考素材，不能安全重试。', 'JOB_REFERENCE_UNAVAILABLE')
        }
        removeQueuedJob(root, job.id, job.profileId)
        // A bridge treats clientJobId as an idempotency key. Reusing the local
        // record's ID after a terminal remote failure would only return the
        // old Bridge job, so retries deliberately create a new local job ID.
        ...
        schedule(root, retried.id, retried.profileId)
        responseJson(res, 202, { job: publicJob(retried) })
        return true
      }
