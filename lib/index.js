...
function normalizeSshConfig(value = {}) {
  ...
  const config = {
    ...
    identityFile: typeof value.identityFile === 'string' ? value.identityFile.trim() : '',
  ...
  if (config.identityFile && (!path.isAbsolute(config.identityFile) || /[\0\r\n]/u.test(config.identityFile))) {
    throw new WorkbenchError('SSH 私钥路径必须是本机绝对路径。')
  }
  return config
}
...
async function startSshTunnel(rawConfig) {
  const config = await writeSshConfig(rawConfig)
  if (sshProcess && !sshProcess.killed) return sshPublicConfig(config, await getSshStatus(config))
  ...
  sshProcessError = ''
  sshProcess = spawn('ssh', args, { stdio: ['ignore', 'ignore', 'pipe'] })
  sshProcess.stderr.setEncoding('utf8')
  sshProcess.stderr.on('data', chunk => { sshProcessError = String(chunk).trim().slice(-500) })
  sshProcess.once('error', error => { sshProcessError = errorMessage(error) })
  sshProcess.once('close', code => {
    if (code !== 0) sshProcessError = sshProcessError || `ssh 已退出（${code}）`
    sshProcess = null
  })
  return sshPublicConfig(config, await getSshStatus(config))
}
...
async function handleSshRequest(req, res, url) {
  ...
  if (url.pathname === '/ai-drama/api/ssh/start' && req.method === 'POST') {
    const config = Object.keys(body).length ? body : await readSshConfig()
