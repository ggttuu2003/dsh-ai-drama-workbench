...
test('Comfy archive verifies bridge size/checksum before publishing and starts with two disabled server placeholders', async () => {
  ...
  try {
    ...
    assert.equal(sceneImageImg2ImgPreset?.label, '场景图（参考生图）')
    assert.deepEqual(sceneImageImg2ImgPreset?.inputs.map(input => input.key), ['prompt', 'negativePrompt', 'width', 'height', 'seed'])
    assert.equal('denoise' in (sceneImageImg2ImgPreset?.defaults ?? {}), false)
    assert.equal('negativePrompt' in (sceneImageImg2ImgPreset?.defaults ?? {}), true)
    assert.equal(getComfyWorkflowPresets().find(preset => preset.id === 'character-costume-v1'), undefined)
    const characterPresets = getComfyWorkflowPresets().filter(preset => preset.assetTypes?.includes('character'))
    assert.deepEqual(characterPresets.map(preset => preset.id), ['character-turnaround-v1'])

    const firstFrameReferencePreset = getComfyWorkflowPresets().find(preset => preset.id === 'shot-first-frame-img2img-v1')
    assert.deepEqual(firstFrameReferencePreset?.inputs.map(input => input.key), ['prompt', 'negativePrompt', 'width', 'height', 'seed'])
