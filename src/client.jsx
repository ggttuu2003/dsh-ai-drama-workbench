...
function DramaWorkbenchShell() {
  ...
  const sshAction = useCallback(async (endpoint, body = {}) => {
    ...
    try {
      ...
      setSsh(data); setSshError('')
    } catch (error) { setSshError(error instanceof Error ? error.message : 'SSH 操作失败'); await loadSsh() }
  ...
  return <>
    ...
    <nav aria-label="工作台快捷入口" className="adw-top-tools">
      <button className="adw-top-tool-button" onClick={() => window.dispatchEvent(new CustomEvent('ai-drama:open-project-structure'))} type="button"><span aria-hidden="true">▤</span>目录预览</button>
    ...
    {sshOpen ? <SshSettings connection={ssh} error={sshError} busy={sshBusy} onClose={() => setSshOpen(false)} onRefresh={() => void loadSsh()} onSave={value => void sshAction('config', value)} onStart={value => void sshAction('start', value)} onStop={() => void sshAction('stop')} /> : null}
