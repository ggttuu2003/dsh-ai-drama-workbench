...
function parseAssetAction(value) {
  ...
  switch (value.action) {
    ...
    case 'prepareSceneImageFromShot':
      return isText(value.shotPath) ? { action: value.action, shotPath: value.shotPath } : null
    case 'createLocation':
    case 'createProp':
      return isText(value.name) ? { action: value.action, name: value.name } : null
    case 'createShot':
...
async function performAssetAction(action) {
  switch (action.action) {
    ...
    case 'prepareSceneImageFromShot': return { ok: true, path: await prepareSceneImageFromShot(action.shotPath) }
