...
type ActiveTab = "characters" | "locations" | "props" | "shots" | "assembly";
...
function displayWorkspaceAssetTitle(asset: WorkspaceSelectionAsset): string {
  ...
  if (asset.type === "scene") return `${asset.sceneId} · 场次`;
  return `${asset.design.shotId} · ${displayShotTitle(asset)}`;
}

function normalizeLegacySceneDocumentTitle(content: string, sceneId: string): string {
...
type ShotReferenceVisual = {
  ...
  label: string;
  detail: string;
  /** Semantic upload role used by multi-reference workflows. */
  role?: string;
  file: AssetFile;
};

type FrameGenerationTarget = "sceneReference" | "firstFrame" | "lastFrame";
...
function frameGenerationLabel(target: FrameGenerationTarget): string {
...
type AssemblyVideoChoice = AssetFile & {
  slotKey: string;
  slotLabel: string;
};
...
function normalizeAssemblyData(value: unknown): AssemblyData {
  const source = value && typeof value === "object" && !Array.isArray(value)
    ? value as Record<string, unknown>
    : {};
  const slotLabels: Record<string, string> = { video: "视频", final: "定稿", candidate: "候选" };
  const shots = Array.isArray(source.shots) ? source.shots.flatMap((value) => {
    if (!value || typeof value !== "object" || Array.isArray(value)) return [];
    const item = value as Record<string, unknown>;
    if (typeof item.shotPath !== "string" || typeof item.sceneId !== "string" || typeof item.shotId !== "string") return [];
    // Accept both the API's `choices` shape and the planner's `candidates`
    // shape so a freshly updated host does not render every shot as missing.
    ...
    const choices = Array.isArray(rawChoices) ? rawChoices.flatMap((choice) => {
      ...
      const file = choice as Record<string, unknown>;
      if (typeof file.path !== "string" || typeof file.name !== "string" || file.kind !== "video") return [];
      const slotKey = typeof file.slotKey === "string"
        ? file.slotKey
        : typeof file.slot === "string" ? file.slot : "candidate";
      return [{
        name: file.name,
        path: file.path,
        kind: "video" as const,
    ...
    const selected = item.selected && typeof item.selected === "object" && !Array.isArray(item.selected)
      ? item.selected as Record<string, unknown>
      : undefined;
    const selectedPath = typeof item.defaultVideoPath === "string"
      ? item.defaultVideoPath
      : selected && typeof selected.path === "string" ? selected.path : undefined;
    const validSelectedPath = selectedPath && choices.some((choice) => choice.path === selectedPath)
      ? selectedPath
      : undefined;
    return [{
  ...
  const outputs = Array.isArray(source.outputs) ? source.outputs.flatMap((value) => {
    if (!value || typeof value !== "object" || Array.isArray(value)) return [];
...
function AssemblyPage({
  ...
  onMerge: () => void;
  onRefresh: () => void;
  onSelect: (shotPath: string, videoPath: string) => void;
}) {
  ...
  return <section aria-labelledby="assembly-title" className="assembly-page" id="assembly-panel">
    <header className="assembly-page-heading">
      ...
      <div className="assembly-page-actions">
        ...
        <button className="submit-button" disabled={!canMerge} onClick={onMerge} type="button">合并成片</button>
      </div>
    </header>
    ...
    {loading && !data ? <div className="assembly-empty">正在读取分镜视频…</div> : shots.length ? <div className="assembly-shot-list">
      {shots.map((shot, index) => {
        ...
        return <article className={`assembly-shot-row ${selected ? "has-video" : "is-missing"}`} key={shot.shotPath}>
          <span className="assembly-shot-index">{String(index + 1).padStart(2, "0")}</span>
          <div className="assembly-shot-copy">
            <strong>{shot.sceneId} / {shot.shotId}</strong>
            <span>{shot.title}</span>
          </div>
          {selected ? <video className="assembly-shot-preview" controls muted playsInline preload="none" src={mediaUrl(selected)} /> : <span className="assembly-shot-missing">缺少视频</span>}
          <div className="assembly-shot-choice">
            {shot.choices.length > 1 ? <select aria-label={`选择${shot.sceneId} ${shot.shotId}的视频`} disabled={busy} onChange={(event) => onSelect(shot.shotPath, event.target.value)} value={selected?.path || ""}>
              {shot.choices.map((choice) => <option key={choice.path} value={choice.path}>{choice.slotLabel} · {choice.name}</option>)}
            </select> : <span title={selected?.name}>{selected?.name || "—"}</span>}
          </div>
        </article>;
      })}
    ...
    {data?.outputs?.length ? <section className="assembly-history" aria-label="历史总片">
      ...
      <div className="assembly-history-list">{data.outputs.map((file) => <article className="assembly-history-item" key={file.path}>
        <video controls playsInline preload="none" src={mediaUrl(file)} />
        <div><strong title={file.name}>{file.name}</strong><small>{formatTimestamp(file.updatedAt)} · {formatSize(file.size)}</small></div>
      </article>)}</div>
    </section> : null}
  </section>;
}
...
type ComfyPreset = {
  ...
  /** Maximum number of frame reference images accepted by this preset. */
  maxReferenceImages?: number;
  /** Ordered upload slots exposed by a multi-reference workflow. */
...
function GenerationModal({
  ...
  // A frame/reference generation request may switch only between its matching
  // text-to-image and image-to-image presets. Other workflow entries stay fixed.
  ...
  // servers and cap the UI at two inputs for the current frame workflow.
  ...
  const advertisedReferenceLimit = advertisedReferenceLimits.length
    ? Math.min(...advertisedReferenceLimits)
    : undefined;
  const maxReferenceImages = isFrameImageToImage
    ? Math.min(2, Math.max(1, advertisedReferenceLimit || 1))
    : 0;
  const supportsMultipleReferences = maxReferenceImages > 1;
  const selectedReferenceImages = useMemo(() => referenceImagePaths.map((path, index) => ({
    path,
    ...
    role: activePreset?.referenceImageRoles?.[index]
  ...
  // single-image contract while the dialog is open. Keep stale selections out
  // of the next request instead of submitting an array the preset cannot use.
  useEffect(() => {
  ...
  // Older Bridge instances may advertise only the text-to-image half of a
  // frame flow. Fall back to the available mode so the dialog does not open
  // with an unavailable preset and a permanently disabled submit button.
  useEffect(() => {
    ...
    const fallbackMode: FrameGenerationMode = frameMode === "imageToImage" ? "textToImage" : "imageToImage";
  ...
  const frameModeAvailable = (mode: FrameGenerationMode) => !frameGeneration || availablePresets.some((preset) => (
    preset.id === framePresetId(frameGeneration.target, mode)
    && (mode !== "imageToImage" || preset.referenceImagesEnabled)
  ));
  ...
  const toggleReferenceImage = (candidate: ShotReferenceVisual) => {
    ...
    setReferenceImagePaths((current) => {
      ...
      // Once the advertised limit is reached, require an explicit deselection
      // before adding another image so a click never silently drops a choice.
        ...
        {loading ? <p className="generation-loading">正在读取生成设置…</p> : <>
          ...
          {frameGeneration ? <section className="generation-section frame-generation-section">
            <div aria-label={`${frameGenerationLabel(frameGeneration.target)}生成方式`} className="frame-generation-mode-toggle" role="group">
              ...
              <button aria-pressed={frameMode === "imageToImage"} disabled={!frameModeAvailable("imageToImage")} onClick={() => chooseFrameMode("imageToImage")} type="button">参考生图</button>
            </div>
            {isFrameImageToImage ? <div className="frame-reference-picker">
              <div className="frame-reference-picker-heading">
                <strong>{supportsMultipleReferences ? "参考图" : "输入图"}</strong>
                <small>{referenceImagePaths.length}/{maxReferenceImages}</small>
              </div>
              {frameGeneration.candidates.length ? <div className="frame-reference-grid">
                {frameGeneration.candidates.map((candidate) => {
                  ...
                  >
                    ...
                    <span className="frame-reference-option-copy">
                      <strong title={candidate.label}>{candidate.label}</strong>
                      <small>{candidate.detail}</small>
                    </span>
                    {selected ? <span aria-hidden="true" className="frame-reference-option-check">{selectedIndex + 1}</span> : null}
                  </button>;
                })}
              </div> : <p className="frame-reference-empty">没有可用输入图</p>}
            </div> : null}
          </section> : null}
          {activePreset ? <section className="generation-section generation-prompt-section">
            ...
            {supportsInput("negativePrompt") ? <details className="generation-secondary-prompt"><summary>负面提示词</summary><TextField label="负面提示词" multiline onChange={(value) => { promptEditedRef.current = true; setNegativePromptDraft(value); setPreview(null); }} placeholder={promptLoading ? "正在读取，可直接输入" : "输入负面提示词"} value={negativePromptDraft ?? ""} /></details> : null}
          </section> : null}
          <details className="generation-section generation-options">
            ...
            {supportsInput("seed") ? <TextField label="Seed" onChange={setSeed} value={seed} /> : null}
          </details>
...
export function Workbench({ externalStructureTrigger = false }: { externalStructureTrigger?: boolean } = {}) {
  ...
  const [pendingSceneImageCandidates, setPendingSceneImageCandidates] = useState<ShotReferenceVisual[]>([]);
  ...
  const [assemblySelections, setAssemblySelections] = useState<Record<string, string>>({});
  ...
  const loadAssembly = useCallback(async (
    ...
    try {
      ...
      if (!response.ok) throw new Error((payload as { error?: string }).error || "无法读取成片素材");
  ...
  const localAssemblyReadyCount = localAssemblyShots.filter((shot) => shot.slots.some((slot) => (
    ["video", "final", "candidate"].includes(slot.key) && slot.files.some(isVideo)
  ))).length;
    ...
    const source = activeTab === "characters" ? characterAssets
      : activeTab === "locations" ? locationAssets
        : activeTab === "props" ? propAssets
          : activeTab === "shots" ? activeShotAssets
            : [];
  ...
  const selectedShotReferenceVisuals = useMemo<ShotReferenceVisual[]>(() => {
    ...
    const seen = new Set<string>();
    const addVisual = (visual: ShotReferenceVisual) => {
      if (seen.has(visual.file.path)) return;
      seen.add(visual.file.path);
      visuals.push(visual);
    };
    const sceneFile = activeScene?.scene
  ...
  const frameGenerationCandidates = (target: FrameGenerationTarget): ShotReferenceVisual[] => {
    ...
    const seen = new Set<string>();
    const add = (label: string, detail: string, file: AssetFile | undefined, role?: string) => {
  ...
  const sceneAssetGenerationCandidates = (targetAsset: WorkspaceSelectionAsset): ShotReferenceVisual[] => {
    ...
    const roleOrder: Record<string, number> = { sceneReference: 0, characterReference: 1, propReference: 2 };
    return candidates.sort((left, right) => (
  ...
  const handleGenerationJobsObserved = useCallback((assetPath: string, jobs: ComfyJob[]) => {
    ...
    if (result.archivedCount > 0) {
      ...
      // Both text-to-image and image-to-image scene presets archive into the
      // location's 场景图 slot. Match the shared label prefix so either result
      // is promoted to the current reference automatically.
      const sceneImageJob = result.archivedJobs.find((job) => (
        (job.presetLabel || "").startsWith("场景图")
        && job.outputPaths?.some((path) => path.startsWith(`${assetPath}/场景图/`))
      ));
  ...
  const openAssetGeneration = async () => {
    if (!selectedAsset) return;
    const frameGeneration = selectedAsset.type === "location" || selectedAsset.type === "scene"
      ? { target: "sceneReference" as const, candidates: sceneAssetGenerationCandidates(selectedAsset) }
      : undefined;
    await openGeneration(undefined, false, frameGeneration);
  };
  ...
  const handleGenerateSceneImageFromShot = async () => {
    ...
    // `handleSave()` are asynchronous, but the draft already contains any new
    // character override the user just added.
    const referenceCandidates = frameGenerationCandidates("sceneReference");
    if (hasUnsavedShotDesign && !(await handleSave())) return;

    // Keep the shot's selected person/scene/prop visuals with the request while
    // the reusable location asset is being prepared and the snapshot refreshes.
  ...
  const handleAssemblySelection = useCallback((shotPath: string, videoPath: string) => {
    setAssemblySelections((current) => ({ ...current, [shotPath]: videoPath }));
  }, []);

  const handleMergeAssembly = useCallback(async () => {
    ...
    try {
      const selections = assemblyData.shots.map((shot) => ({
        ...
        videoPath: shot.choices.some((choice) => choice.path === assemblySelections[shot.shotPath])
          ? assemblySelections[shot.shotPath]
          : shot.choices.some((choice) => choice.path === shot.defaultVideoPath)
            ? shot.defaultVideoPath
            : shot.choices[0]?.path,
      }));
      ...
      const payload = await response.json().catch(() => ({})) as { error?: string; file?: { name?: string; path?: string } };
    ...
    <main className="workbench-shell">
      ...
      <div className="asset-workspace-grid">
        <aside className="asset-library-rail">
          <div className="asset-rail-heading"><h1>工作台</h1></div>
          <div className="asset-tabs" role="tablist" aria-label="分镜制作与资产库">
            <div className="asset-nav-group">
              <span className="asset-nav-group-label">分镜制作</span>
              <button aria-controls="asset-list-panel" aria-selected={activeTab === "shots"} className={`is-primary-tab ${activeTab === "shots" ? "is-active" : ""}`} id="shots-tab" onClick={() => changeTab("shots")} role="tab" tabIndex={activeTab === "shots" ? 0 : -1} type="button"><span>场次</span><b>{sceneGroups.length}</b></button>
              <button aria-controls="assembly-panel" aria-selected={activeTab === "assembly"} className={`is-primary-tab ${activeTab === "assembly" ? "is-active" : ""}`} id="assembly-tab" onClick={() => changeTab("assembly")} role="tab" tabIndex={activeTab === "assembly" ? 0 : -1} type="button"><span>成片</span><b>{assemblyData ? `${assemblyData.shots.filter((shot) => shot.choices.length).length}/${assemblyData.shots.length}` : `${localAssemblyReadyCount}/${localAssemblyShots.length}`}</b></button>
            </div>
            <div className="asset-nav-group">
        ...
        <section className="asset-studio-column">
          <div className={`asset-studio-head asset-studio-toolbar ${activeTab === "assembly" ? "is-assembly-studio" : ""} ${selectedAsset?.type === "character" ? "is-character-studio" : ""} ${selectedAsset?.type === "scene" ? "is-scene-studio" : ""}`}>
            <div>
              <p className="asset-breadcrumb">{activeTab === "assembly" ? "分镜制作 / 成片" : selectedAsset?.type === "location" ? "资产库 / 场景" : selectedAsset?.type === "prop" ? "资产库 / 道具" : activeTab === "characters" ? selectedAsset?.type === "character" ? `资产库 / 人物 / ${selectedAsset.roleCategory}` : "资产库 / 人物" : activeScene ? `分镜制作 / 场次 / ${activeScene.sceneId}` : "分镜制作"}</p>
              <div className="asset-title-row">
                <h2>{activeTab === "assembly" ? "成片" : selectedAsset ? displayWorkspaceAssetTitle(selectedAsset) : "选择资产"}</h2>
                {activeTab !== "assembly" && selectedAsset?.type === "character" ? <span
          ...
          {activeTab !== "assembly" && selectedAsset && revisionConflictKey === assetKey(selectedAsset) ? <section className="revision-conflict-notice" role="alert">
            <div>
          ...
          {activeTab === "assembly" ? <AssemblyPage
            busy={busy}
            data={assemblyData}
            error={assemblyError}
            ...
            <div className={`shot-editor ${selectedAsset.isDraft ? "" : "is-workflow"}`}>
              ...
              {selectedAsset.isDraft || activeShotWorkflowStep === "design" ? <section className="editor-card shot-design-editor"><div className="editor-card-heading"><div><h3>{selectedAsset.isDraft ? "分镜草稿" : "分镜描述"}</h3></div>{selectedAsset.isDraft ? <button className="save-button primary" disabled={busy || !selectedAsset.sourcePath} onClick={handleCreateSelectedDraft} type="button">{busy ? "创建中…" : "新建分镜"}</button> : <div className="editor-card-heading-actions"><button aria-pressed={shotDesignMode === "edit"} className="editor-mode-button" onClick={() => setShotDesignMode((mode) => mode === "preview" ? "edit" : "preview")} type="button">{shotDesignMode === "preview" ? "修改" : "预览"}</button>{shotDesignMode === "edit" ? <button className="save-button" disabled={busy || !hasUnsavedShotDesign} onClick={() => void handleSave()} type="button">{busy ? "处理中…" : hasUnsavedShotDesign ? "保存" : "已保存"}</button> : null}</div>}</div>
                {selectedAsset.isDraft ? <DraftSummary design={designDraft} /> : shotDesignMode === "preview" ? <ShotDesignPreview design={designDraft} /> : <>
                  ...
                  <TextField label="视频生成提示词" multiline onChange={(value) => setDesignDraft((draft) => ({ ...draft, videoPrompt: value }))} placeholder="输入视频生成专用提示词（可选）" value={designDraft.videoPrompt} />
      ...
      {modal === "generation" && generationTarget ? <GenerationModal
        ...
        // Reset frame mode and selected references when the target changes;
        // otherwise a still-mounted dialog can carry a person's choices into
        // a different shot, frame, or reusable scene asset.
        ...
        onClose={() => { setModal(null); setGenerationTarget(null); setGenerationFrame(null); setPendingSceneImageCandidates([]); setGenerationDurationSeconds(undefined); setGenerationPresetId(undefined); }}
        ...
        <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setModal(null); }}>
          <section aria-labelledby="asset-modal-title" aria-modal="true" className="modal-card asset-modal" role="dialog">
            ...
            {modal === "projectSettings" ? <>
              ...
              <div className="modal-actions"><button className="text-button" onClick={() => setModal(null)} type="button">取消</button><button className="submit-button" disabled={busy || !newName.trim()} onClick={() => void handleCreateCharacter()} type="button">新建人物</button></div>
              ...
              <div className="modal-actions"><button className="text-button" onClick={() => setModal(null)} type="button">取消</button><button className="submit-button" disabled={busy || !newSimpleAssetName.trim()} onClick={() => void handleCreateSimpleAsset(modal)} type="button">{modal === "location" ? "新建场景" : "新建道具"}</button></div>
            </> : modal === "look" ? <>
              <TextField label="造型名称" onChange={setNewLookName} placeholder="例如：边关黑衣僧" value={newLookName} />
              <div className="modal-actions"><button className="text-button" onClick={() => setModal(null)} type="button">取消</button><button className="submit-button" disabled={busy || !newLookName.trim()} onClick={() => void handleCreateCharacterLook()} type="button">新建造型</button></div>
            </> : modal === "scene" ? <>
