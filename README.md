...
- 工作台中的人物、场景、道具和基础镜头图片默认是纯文生图：只读取当前已保存的 Markdown，不会因为资料槽存在 `-已选` 图片就自动上传。镜头首帧/尾帧另有明确的图生图预设：首帧优先读取镜头参考图，其次读取当前场次绑定的场景、道具或人物已选视觉图，再回退旧场次资料槽；尾帧固定读取已选首帧。视频工作流仍会按要求读取已选首帧和尾帧。任务通过本机服务安全提交到云端 Comfy Bridge；结果会自动格式化并归档到对应资料槽，绝不覆盖现有定稿。
...
更新 Bridge 时，在同一弹窗输入 SSH 密码并点击“同步工作流”。工作台会把内置 `bridge.py`、`run.sh`、工作流契约和 ComfyUI API 工作流同步到固定目录 `/root/comfy-bridge`，自动重启 Bridge、建立所需隧道并校验全部工作流。同步只覆盖同名程序与工作流文件，不会上传或覆盖远端 `.env`、`data/` 和任务记录；SSH 密码仍只用于本次操作。
...
本机工作台重启时，会自动恢复尚未提交的本地队列及已有远端编号的轮询；上传、下载、归档等无法确定是否完成的中间态会直接标为失败，需要用户重新生成，不会盲目重复提交或覆盖资料。下载后会校验 Bridge 声明的文件大小和 SHA-256，再原子归档到资料槽。

## 回收站与候选图整理

- “移入回收站”不会直接删除文件。每个新移入的项目会记录其原始相对路径；点击右上角“回收站”可查看并恢复。
- 恢复不会覆盖任何现有文件。若先恢复了资料、后恢复其所属人物或镜头导致上级目录不存在，先恢复上级资产即可。
- `-已选` 是文件名中的持久选择状态。请通过“设为参考 / 统一选此图”切换，而不是手动复制定稿；图生图与视频工作流会读取生成弹窗校验的已选素材，纯文生图不会上传它。
