...
工作台顶部的“云服务器”可管理本机 SSH 隧道，不需要每次手动输入转发命令。填写服务器地址、SSH 用户、私钥路径（可选）、本地端口及云端服务端口后，点击“启动连接”。工作台会以 `ssh -N -L` 建立只绑定 `127.0.0.1` 的本地转发，并每 10 秒检查隧道进程和本地端口状态。

配置保存在本机 `~/.dsh/ai-drama-workbench-ssh.json`（或 `$DSH_HOME/ai-drama-workbench-ssh.json`），权限为 `0600`。文件只记录私钥路径，不保存私钥内容；停止或关闭插件后需按需重新启动隧道。
