...
所有图片任务的生成弹窗都会显示提示词和负面提示词；用户可在提交前临时修改，只影响本次任务，不回写人物、地点/环境或镜头 Markdown。图生视频仍只读取已保存的镜头资料。

1. 镜头设计：保存画面、台词、景别、运镜、时长、提示词和人物造型覆盖。
2. 画面参考：可上传镜头参考图，也可使用已绑定地点/环境资产或人物已选视觉图。在当前镜头点击“生成场景图”时，系统创建或复用项目级 `场景/<场次名>` 地点/环境资产，将已保存的镜头 `prompt` 或 `content` 写入该资产的场景设定，并自动绑定到当前分镜场次。生成结果归档到该地点/环境资产的场景图槽，不生成或使用分镜场次资料槽。生成候选仍须用户明确设为参考后才可作为首帧参考。这是可选输入，不应阻断只有文字设定的镜头。
