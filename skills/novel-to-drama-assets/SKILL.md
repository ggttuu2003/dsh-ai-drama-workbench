...
2. 画面参考：可上传镜头参考图，也可继承场次场景图或人物已选视觉图。如果当前场次尚无场景图，优先留在当前镜头使用“生成场景图”：系统会以已保存的镜头 `prompt` 或 `content` 初始化未配置的场次资产，并把生成结果归档到本场场景图槽；已有场次说明不得覆盖。生成候选仍须用户明确标为“已选”后才可作为首帧参考。这是可选输入，不应阻断只有文字设定的镜头。
